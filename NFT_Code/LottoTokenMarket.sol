...
contract LotteryTokenMarketPlace is Ownable {
    ...
    mapping(uint256 => LotteryToken) public tokendict;
    ...
    address payable private addr = address(uint160(address(this)));
    ...
    function _processPurchase(uint256 amount, address payable purchaser) internal returns (uint256) {
        return _token.newLottoToken(amount,purchaser,addr);
    }
    
    function _forwardFunds(uint256 tokenValue, address payable beneficiary) internal {
        if (_price > 0) {
       // _wallet.transfer(_price);
        }

        if (tokenValue > 0) {
        beneficiary.transfer(tokenValue);
        }
    }
    //function makenewtoken(uint tokenId) public {
      //  tokendict[tokenId] = _token.newLottoToken(amount,purchaser,addr);
    //}
